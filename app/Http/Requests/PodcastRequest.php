    ...
    public function rules()
    {
        ...
        if (!$this->cmnd)
            $tmp = array_merge($tmp, [
                ...
                'name' => ['required', 'max:100', Rule::unique('podcasts', 'name')->ignore($this->id)],
                'narrator' => ['nullable', 'max:100',],
        ...
        if ($request->uploading)
            $tmp = array_merge($tmp, [
                ...
                'podcast' => ['required', 'mimes:' . implode(",", Variable::PODCAST_ALLOWED_MIMES)],
        ...
        if ($this->cmnd)
            $tmp = array_merge($tmp, [
                ...
                'podcast' => ['sometimes',/* File::types(['mp3', 'wav']) ->min(1024) ->max(12 * 1024),*/
                    'mimes:' . implode(",", Variable::PODCAST_ALLOWED_MIMES)],
