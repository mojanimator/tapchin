    ...
    <template v-slot:content>
      ...
      <div
        ...
        <div class="flex">
          ...
          <h1 class="text-2xl font-semibold">{{ __('transfers_list') }}</h1>
        </div>
        <div>
          <Link :href="route('panel.transfer.create')"
          ...
          <table class="w-full text-sm text-left text-gray-500">
            ...
            <tr class="text-sm text-center">
              ...
              <th scope="col"
                ...
                <div class="flex items-center justify-center">
                  <span class="px-2">    {{ __('sell_price') }} </span>
                  <ArrowsUpDownIcon class="w-4 h-4 "/>
            ...
            <tr v-for="(d,idx) in data"
              ...
              <td
                ...
                <Link class="px-3 hover:text-gray-500" :href="route('panel.transfer.edit',d.id)">
                  <div class="text-base font-semibold">{{ cropText(d.item_name, 30) }}</div>
                  <div class="font-normal text-gray-500">{{ }}</div>
                </Link>
              </td>
              ...
              <td class="px-2 py-4    " data-te-dropdown-ref>
                <button
                    ...
                    :class="`bg-${getStatus('transfer', d.status).color}-100 hover:bg-${getStatus('transfer', d.status).color}-200 text-${getStatus('transfer', d.status).color}-500`">
                  {{ getStatus('transfer', d.status).name }}
                </button>
                <ul ref="statusMenu" data-te-dropdown-menu-ref
                  ...
                  <li v-if="d.status=='active'  " role="menuitem"
                      @click="edit({'idx':idx,'id':d.id,'item_id':d.item_id,'item_type':d.item_type,'cmnd':'inactive'})"
                  ...
                  <li v-if="d.status=='inactive'  " role="menuitem"
                      @click="edit({'idx':idx,'id':d.id,'item_id':d.item_id,'item_type':d.item_type,'cmnd':'active'})"
              ...
              <td class="px-2 py-4">
                ...
                <div
                    ...
                    role="group">
                  <Link v-if="d.status!='done'"
                        type="button" :href="route('panel.transfer.edit',d.id)"
                        class="inline-block grow rounded-s  bg-orange-500 text-white px-6  py-2 text-xs font-medium uppercase leading-normal text-white transition duration-150 ease-in-out hover:bg-orange-400   focus:outline-none focus:ring-0  "
                        data-te-ripple-init
                        data-te-ripple-color="light">
                    {{ __('edit') }}
                  </Link>
                  <button v-if="d.status!='done'"
                          type="button"
                          @click.prevent="showDialog('danger',__('remove_item?'),__('remove'),remove,{id:d.id,idx:idx})"
                          class="inline-block grow rounded-e  bg-danger-500 text-white px-6  py-2 text-xs font-medium uppercase leading-normal text-white transition duration-150 ease-in-out hover:bg-danger-400   focus:outline-none focus:ring-0  "
                          data-te-ripple-init
                          data-te-ripple-color="light">
                    {{ __('remove') }}
                  </button>
                  <Link v-if="d.status=='done'"
                        type="button" :href="route('panel.transfer.edit',d.id)"
                        class="w-full   rounded   bg-primary-500 text-white px-6  py-2 text-xs font-medium uppercase leading-normal text-white transition duration-150 ease-in-out hover:bg-primary-400   focus:outline-none focus:ring-0  "
                        data-te-ripple-init
                        data-te-ripple-color="light">
                    {{ __('details') }}
                  </Link>
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.loading = true;
      this.data = [];
      window.axios.get(route('panel.transfer.search'), {
        params: this.params
      }, {
    ...
    remove(params) {
      this.isLoading(true);
      window.axios.delete(route('panel.transfer.delete', params.id), {},
          {
            onUploadProgress: function (axiosProgressEvent) {
            },
            onDownloadProgress: function (axiosProgressEvent) {
