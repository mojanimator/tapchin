    ...
    <template v-slot:content>
      ...
      <div
        ...
        <div class="flex">
          ...
          <h5 class="  font-semibold">{{ __('packs_list') }}</h5>
        </div>
        <div>
          <Link :href="route('admin.panel.pack.create')"
        ...
        <div class="flex-col   bg-white  overflow-x-auto shadow-lg  rounded-lg">
          ...
          <table class="w-full text-sm text-left text-gray-500">
            ...
            <tr class="text-sm text-center">
              ...
              <th scope="col"
                  class="px-2 py-3   cursor-pointer duration-300 hover:text-gray-500 hover:scale-[105%]"
                  @click="params.order_by='length';params.dir=params.dir=='ASC'? 'DESC':'ASC'; params.page=1;getData()">
                <div class="flex items-center justify-center">
                  <span class="px-2">    {{ __('length') }} </span>
                  <ArrowsUpDownIcon class="w-4 h-4 "/>
                </div>
              </th>
              <th scope="col"
                  class="px-2 py-3   cursor-pointer duration-300 hover:text-gray-500 hover:scale-[105%]"
                  @click="params.order_by='width';params.dir=params.dir=='ASC'? 'DESC':'ASC'; params.page=1;getData()">
                <div class="flex items-center justify-center">
                  <span class="px-2">    {{ __('width') }} </span>
                  <ArrowsUpDownIcon class="w-4 h-4 "/>
                </div>
              </th>
              <th scope="col"
                  class="px-2 py-3   cursor-pointer duration-300 hover:text-gray-500 hover:scale-[105%]"
                  @click="params.order_by='height';params.dir=params.dir=='ASC'? 'DESC':'ASC'; params.page=1;getData()">
                <div class="flex items-center justify-center">
                  <span class="px-2">    {{ __('height') }} </span>
                  <ArrowsUpDownIcon class="w-4 h-4 "/>
                </div>
              </th>
            ...
            <tr v-for="(d,idx) in data"
              ...
              <td
                ...
                <!--                <Image class="w-10 h-10 cursor-pointer rounded-full" :src="`${route('storage.packs')}/${d.id}.jpg`"-->
                <!--                       :data-lity="`${route('storage.packs')}/${d.id}.jpg`"-->
                <!--                       :alt="cropText(d.title,5)"/>-->
                <Link class="px-2 hover:text-gray-500" :href="route('admin.panel.pack.edit',d.id)">
              ...
              <td class="px-2 py-4    " data-te-dropdown-ref>
                <button
                    ...
                    :class="`bg-${getStatus('pack', d.status).color}-100 hover:bg-${getStatus('pack', d.status).color}-200 text-${getStatus('pack', d.status).color}-500`">
                  {{ getStatus('pack', d.status).name }}
                </button>
              ...
              <td class="px-2 py-4">
                ...
                <div
                    ...
                    role="group">
                  <Link
                      type="button" :href="route('admin.panel.pack.edit',d.id)"
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.loading = true;
      this.data = [];
      window.axios.get(route('admin.panel.pack.search'), {
        params: this.params
      }, {})
          .then((response) => {
