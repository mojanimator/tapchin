    ...
    <template v-slot:content>
      ...
      <div
        ...
        <div class="flex">
          ...
          <h1 class="text-2xl font-semibold">{{ __('projects_list') }}</h1>
        </div>
        <div>
          <Link :href="route(`page.project.create`)"
        ...
        <div class="flex-col   bg-white  overflow-x-auto shadow-lg  rounded-lg">
          ...
          <table class="w-full text-sm text-left text-gray-500">
            ...
            <tr class="text-sm text-center">
              <th v-if="false" scope="col" class="p-4" @click="toggleAll">
                <div class="flex items-center">
            ...
            <tr v-if="loading" v-for="i in 3"
              ...
              <td
                  class="flex justify-center items-center px-6 py-4 text-gray-900 whitespace-nowrap">
            ...
            <tr v-for="(d,idx) in data" @click="$inertia.visit(route('panel.project.edit',d.id))"
              ...
              <td
                ...
                <Link class="px-3 hover:text-gray-500" :href="route('panel.project.edit',d.id)">
              ...
              <td class="px-2 py-4     ">
                <div
                    ...
                    :class="`bg-${getStatus('project', d.status).color}-100 hover:bg-${getStatus('project', d.status).color}-200 text-${getStatus('project', d.status).color}-500`">
                  {{ getStatus('project', d.status).name }}
                </div>
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.loading = true;
      this.data = [];
      window.axios.get(route('panel.project.search'), {
        params: this.params
      }, {
