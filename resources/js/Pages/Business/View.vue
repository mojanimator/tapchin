  ...
  <Scaffold navbar-theme="light">
      ...
      <div
        ...
        <div v-else-if="data" class=" flex flex-col ">
          ...
          <div v-if="data" class="    p-4 flex flex-col  m-4">
            ...
            <div class="  p-4 ">
              <div v-if="data.socials" v-for="social in JSON.parse(data.socials)">
                <div class="flex justify-start items-center p-2 border-b">
                  <div class="text-gray-500 mx-2 ">{{ social.name }}:</div>
                  <a :href=" /^https?:\/\//i.test(social.value)?social.value:`//${social.value}`" target="_blank"
                     class="mx-2 text-gray-900 cursor-pointer hover:text-gray-500 duration-200">{{
                      social.value
                    }}
                  </a>
                </div>
    ...
    <section>
      <div class=" w-full px-3 my-8   flex  items-center justify-center">
        <PrimaryButton @click="$inertia.visit(route(($page.props.auth.user?'panel.business.create':'login')  ))"
